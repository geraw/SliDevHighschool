...
<div class="flex items-center justify-center gap-8" style="direction: rtl; position: absolute; top: 58%; left: 50%; transform: translate(-50%, -50%);">
  ...
  <div style="text-align: right; width: 800px;" class="text-xl leading-relaxed space-y-4">
    ...
    <p v-click>הצוות בסטודיו התכנון הופך את הסקיצה לתוכנית תלת-ממדית מדויקת</p>
...
<div class="flex flex-col items-center justify-center h-full" style="direction: rtl; text-align: center;">
  ...
  <div v-click="1">
    ...
    <div class="flex flex-col space-y-4 mt-6 text-xl leading-relaxed">
      <div v-click="2">🤨 "הכל מתוכנת מראש - זו כוריאוגרפיה קבועה"</div>
      <div v-click="3">😐 "הרחפן לא מגיב אלינו, לקהל שצופה בו"</div>
      <div v-click="4">🔁 "בכל פעם שמפעילים את המופע רואים בדיוק את אותו הדבר"</div>
      <div v-click="5">📼 "מישהו רק לוחץ 'הפעל' - זה כמו סרטון מוקלט"</div>
    </div>
  </div>

  <div v-click="6" class="mt-9">
    <h2>עמדה ב׳: אבל בכל זאת יש כאן תגובתיות!</h2>
    <div class="flex flex-col space-y-4 mt-6 text-xl leading-relaxed">
      <div v-click="7">💨 אם תבוא רוח, הרחפן יתקן את עצמו כדי להישאר יציב</div>
      <div v-click="8">🔋 אם הסוללה תיחלש או יתקלקל רחפן, המערכת תזהה זאת ותגיב בהתאם</div>
    </div>
  </div>
